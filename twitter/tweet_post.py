...
from consts import (
    userID, url_search_std, url_icon, url_update
)
...
def id_to_unix(x):
    return datetime.datetime(1970, 1, 1, 9) \
        + datetime.timedelta(milliseconds=int(x / 2**22) + 1288834974657)
...
def time_check(tweet, twitter):
    ...
    for t, r in zip(range(cnt), result):
        time = id_to_unix(int(r['id']))
        time_str = time.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
        print('{0}: {1} {2}'.format(str(t).rjust(3),
                                    time_str, trim_text(r['text'], 10)[0]))
    print()
...
def tweet_post(id_list, tweet_list, text, offset, twitter):
    ...
    params = {'status': tweet}
    time_push = datetime.datetime.now()\
    ...
    if req.status_code == 200:
        ...
        length = min(len(tweet), 999)
        tweet_list.insert(0, tweet[0:length])
    print('')
    time.sleep(0.5)
...
def tweet_destroy(id_list, tweet_list, tweet, twitter):
    if(len(id_list) == 0):
        ...
        url_destroy = \
            f"httos://api.twitter.com/1.1/statuses/destroy/{id_str}.json"
...
def tweet_edit(id_list, tweet_list, tweet, twitter):
    if(len(id_list) == 0):
        ...
        tweet_after_mirror = tweet_before_mirror.replace(
            str_before_mirror, str_after_mirror, 1)
        ...
        # ツイートを削除した後再投稿
        url_destroy = \
            f"https://api.twitter.com/1.1/statuses/destroy/{id_str}.json"
        req = twitter.post(url_destroy)
        if req.status_code == 200:
            id_list.pop(id_num)
            tweet_list.pop(id_num)
            time.sleep(0.5)
            id_list, tweet_list = tweet_post(
                id_list, tweet_list, tweet_after, twitter)
        else:
            print("Failed: Delete\n")
    return id_list, tweet_list
