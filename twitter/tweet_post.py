...
from consts import userID, url_search_std, url_icon, url_update
...
def id_to_unix(x):
    return datetime.datetime(1970, 1, 1, 9) + datetime.timedelta(
        milliseconds=int(x / 2 ** 22) + 1288834974657
    )
...
def time_check(tweet, twitter):
    ...
    for t, r in zip(range(cnt), result):
        ...
        print(
            "{0}: {1} {2}".format(
                str(t).rjust(3), time_str, trim_text(r["text"], 10)[0]
            )
        )
    print()
...
def tweet_destroy(id_list, tweet_list, tweet, twitter):
    if len(id_list) == 0:
        ...
        url_destroy = f"httos://api.twitter.com/1.1/statuses/destroy/{id_str}.json"
...
def tweet_edit(id_list, tweet_list, tweet, twitter):
    if len(id_list) == 0:
        ...
        # ツイートを削除した後再投稿
        url_destroy = f"https://api.twitter.com/1.1/statuses/destroy/{id_str}.json"
        req = twitter.post(url_destroy)
        if req.status_code == 200:
            id_list.pop(id_num)
            tweet_list.pop(id_num)
